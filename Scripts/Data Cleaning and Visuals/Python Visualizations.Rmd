...
def create_influence_network(dataset1_comments, influential_authors):
    """Create and visualize influence network between authors"""
    
    def get_thread_interactions(group):
        """Get author interactions within a thread"""
        authors = group['Author'].tolist()
        dates = group['Date'].tolist()
        interactions = []
        
        for i in range(1, len(authors)):
            if authors[i] in influential_authors['Author'].values and \
               authors[i-1] in influential_authors['Author'].values:
                interactions.append((authors[i-1], authors[i], 
                                  (dates[i] - dates[i-1]).total_seconds()))
        return interactions

    # Get all interactions between influential authors
    interactions = []
    for _, thread in dataset1_comments.groupby('TH_ID'):
        thread = thread.sort_values('Date')
        interactions.extend(get_thread_interactions(thread))

    # Create network
    G = nx.DiGraph()
    
    # Add nodes
    for author in influential_authors['Author']:
    ...
    for source, target, time in interactions:
        if (source, target) in edge_weights:
            ...
            response_times[(source, target)].append(time)
        else:
            edge_weights[(source, target)] = 1
            response_times[(source, target)] = [time]
    
    for (source, target), weight in edge_weights.items():
        avg_response_time = np.mean(response_times[(source, target)]) / 3600
    ...
    edge_colors = [cmap(rt/max_response) for rt in response_times]

    # Draw network on main axes
    ...
    # Add colorbar
    norm = plt.Normalize(0, max_response)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    plt.colorbar(sm, cax=ax_cbar, label='Average Response Time (hours)')

    # Add legend for node sizes
    sizes = [min(node_sizes), np.median(node_sizes), max(node_sizes)]
    labels = [f'{int(np.exp2(size/100)-1)} comments' for size in sizes]
    ...
    ax_main.set_title('Influence Network of Key Authors\nNode size = total comments, Edge color = response time, Edge thickness = interaction frequency',
                     pad=20, fontsize=14)
    ax_main.axis('off')

    plt.tight_layout()
    plt.savefig('/Users/emerson/Github/usenet_webpage/Images and Tables/Images/influence_network.png',
                dpi=300, bbox_inches='tight')
    plt.close()
...
# Run analysis
stats = create_influence_network(dataset1_comments, influential_authors)
...
for author, degree in stats['most_connected']:
    print(f"{author}: {degree:.0f} interactions")
```
